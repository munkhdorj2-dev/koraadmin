import { VENUE_TYPES, emptyHotelRooms, parseHotelPrices, serializeHotelPrices } from './data.js';
...
export function rowToForm(row) {
  ...
  let hotelStars = '';
  let hotelRooms = emptyHotelRooms();
  let accommodationUnits = [];
  if (row.type === 'hotel' && row.price) {
    try {
      const p = parseHotelPrices(row.price);
      hotelStars = p.stars || '';
      hotelRooms = p.rooms || [];
    } catch { /* plain price */ }
  }
...
export function formToRow(form, type) {
  ...
  if (type === 'hotel') {
    price = serializeHotelPrices(form.hotelRooms, form.hotelStars);
  }
  if (type === 'resort') {
